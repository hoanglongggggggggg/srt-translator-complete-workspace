...
export function ProxySettings() {
    ...
    const triggerGeminiOAuth = async () => {
        setSaving(true);
        showMessage("🔐 Opening Gemini login...");
        try {
            const result = await invoke("trigger_gemini_oauth");
            showMessage(`✅ ${result}`);
            refetch();
        } catch (e) {
            showMessage(`❌ Gemini login failed: ${e}`);
        } finally {
            setSaving(false);
        }
    };
                ...
                {(cfg) => (
                    <>
                        ...
                        <div class="settings-section">
                            ...
                            <button
                                onClick={triggerGeminiOAuth}
                                disabled={saving()}
                                class="btn-oauth google"
                            >
                                <span>🔐</span> Login Google (OAuth)
                            </button>
                        ...
                        <div class="settings-section">
                            ...
                            <div class="oauth-buttons">
                                <button
                                    onClick={() => invoke("trigger_oauth_login", { provider: "copilot" })}
                                    disabled={saving()}
                                    class="btn-oauth copilot"
                                >
                                    <span>🤖</span> GitHub Copilot
                                </button>
                                <button
                                    onClick={() => invoke("trigger_oauth_login", { provider: "claude" })}
                                    disabled={saving()}
                                    class="btn-oauth claude"
                                >
                                    <span>💬</span> Claude (OAuth)
                                </button>
                                <button
                                    onClick={() => invoke("trigger_oauth_login", { provider: "qwen" })}
                                    disabled={saving()}
                        ...
                        {/* ============= App Behavior ============= */}
                        <details class="settings-section">
                            <summary>⚙️ App Behavior</summary>
