...
fn get_clipproxy_path(app: &tauri::AppHandle) -> Result<PathBuf, String> {
    use tauri::Manager;
    ...
    fn normalize_path(path: PathBuf) -> PathBuf {
        ...
        {
            // Convert to string and strip \\?\ prefix if present
    ...
    if !path.exists() {
        if let Ok(resource_dir) = app.path().resource_dir() {
            path = normalize_path(resource_dir.join("clipproxyapi.exe"));
        }
    }
    ...
    {
        if !path.exists() {
            let dev_path = std::env::current_dir().unwrap()
                .join("..").join("proxypal-bundle")
    ...
    if !path.exists() {
        return Err(format!("clipproxyapi.exe not found. Checked: {:?}", path));
    }
...
pub async fn trigger_gemini_oauth(app: tauri::AppHandle) -> Result<String, String> {
    let clipproxy_path = get_clipproxy_path(&app)?;
    
    let output = std::process::Command::new(clipproxy_path)
        .arg("-gemini-login")
        .output()
        .map_err(|e| format!("Failed to run Gemini OAuth: {}", e))?;
    
    if output.status.success() {
        Ok("Gemini login completed successfully".to_string())
    } else {
        Err("Gemini login failed".to_string())
    }
}
...
pub async fn trigger_oauth_login(app: tauri::AppHandle, provider: String) -> Result<String, String> {
    ...
    if output.status.success() {
        Ok(format!("{} login completed", provider))
    } else {
        Err(format!("{} login failed", provider))
    }
}
